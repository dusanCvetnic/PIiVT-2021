...
class ReviewService implements CRUD{
    ...
    async create(req: Request, res: Response){
        ...
        try {
            ...
            res.status(200).send(`Dodata je ocena: ${newReview.rating}, ocenjen je korisnik sa id: ${ratedUserId}, ocenio je korisnik sa id: ${userWhoRatedId}`)
        } catch (error) {
    ...
    async updateById(req: Request, res: Response, resourceId: number){
        ...
        try {
            ...
            SET
                rating = ?
            WHERE 
                review_id = ?;`, [updatedReview.rating, resourceId])

            return `Azurirana je ocena: ${updatedReview.rating}, ocenjen je korisnik sa id: ${req.params.ruid}, ocenio je korisnik sa id: ${req.params.uwrid}`
        } catch (error) {
            res.status(500).send({error: error?.sqlMessage})
        }
    }
